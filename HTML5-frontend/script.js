...
function testDiagnosis() {
  var div = document.createElement('div'),
    diagnosisArray = NDT.testNDT_().get_diagnosis().split('\n'),
    txt = '',
    table,
    isTable = false;
...
function printPacketLoss() {
  var packetLoss = parseFloat(NDT.testNDT_().getNDTvar('loss'));
...
function printDownloadSpeed() {
  var downloadSpeedVal = NDT.downloadSpeed_();
...
function printUploadSpeed() {
  var uploadSpeedVal = NDT.uploadSpeed_(false);
...
function readNDTvar(variable) {
  var ret = NDT.testNDT_().getNDTvar(variable);
  return !ret ? '-' : ret;
}
...
function testDetails() {
  ...
  d += 'Round trip time: ' + readNDTvar('MinRTT').bold() + ' msec (minimum), ' + readNDTvar('MaxRTT').bold() + ' msec (maximum), <b>' + printNumberValue(Math.round(NDT.averageRoundTrip_())) + '</b> msec (average)<br>';
...
function updateGaugeValue() {
  var downloadSpeedVal = NDT.downloadSpeed_(),
    uploadSpeedVal = NDT.uploadSpeed_(false);
...
function setPhase(phase) {
  ...
  case PHASE_UPLOAD:
    ...
    pcBuffSpdLimit = NDT.speedLimit_();
    rtt = NDT.averageRoundTrip_();
    ...
    $('#test .remote.location .address').get(0).innerHTML = NDT.remoteServer_();
    break;
  ...
  case PHASE_RESULTS:
    ...
    printDownloadSpeed();
    printUploadSpeed();
    $('#latency').html(printNumberValue(Math.round(NDT.averageRoundTrip_())));
...
function monitorTest() {
  var message = NDT.testError_(),
    currentStatus = NDT.testStatus_();
  ...
  if (!currentStatus.match(/Middleboxes/) && !currentStatus.match(/notStarted/)
        && !NDT.remoteServer_().match(/ndt/) && currentPhase === PHASE_PREPARING) {
    debug('Remote server is ' + NDT.remoteServer_());
    setPhase(PHASE_UPLOAD);
  }

  if (NDT.remoteServer_() !== 'unknown' && currentPhase < PHASE_PREPARING) {
    setPhase(PHASE_PREPARING);
  }
